...
func _draw() -> void:
	...
	var text_height = font.get_height()
	...
	for i in columns + 1:
		...
		draw_string(font, Vector2(x + margin, min_outer.y - margin), String(stepify(i * x_offset, 0.01)), text_color)
	...
	for i in rows + 1:
		...
		draw_string(font, Vector2(min_inner.x + margin, y - margin), String(stepify(y_value, 0.01)), text_color)
