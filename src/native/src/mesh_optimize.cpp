...
void MeshOptimize::_register_methods() {
  register_method("optimize", &MeshOptimize::optimize);
  register_method("optimize_mesh", &MeshOptimize::optimize_mesh);
  register_method("optimize_mesh_instance", &MeshOptimize::optimize_mesh_instance);
  register_method("simplify", &MeshOptimize::simplify);
}
...
void MeshOptimize::simplify(Node *p_root_node, float p_threshold) {
  Array mesh_items;
...
MeshInstance *MeshOptimize::optimize_mesh_instance(Ref<Mesh> p_mesh, float p_threshold, NodePath p_skeleton_path, String p_name, Ref<Skin> p_skin, bool p_sloppy) {
...
Ref<ArrayMesh> MeshOptimize::optimize_mesh(Ref<Mesh> p_mesh, float p_threshold, bool p_sloppy) {
      struct Vertex {
      float px, py, pz;
      float nx, ny, nz;
      float tx, ty, tz, tw;
...
void MeshOptimize::_find_all_mesh_instances(Array r_items, Node *p_current_node,
                                            const Node *p_owner) {
