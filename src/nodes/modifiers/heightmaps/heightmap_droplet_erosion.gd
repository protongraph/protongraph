...
func _init() -> void:
	...
	set_input(2, "Iterations", ConceptGraphDataType.SCALAR, 
		{"min": 0, "allow_lesser": false, "value": 1000})
	
	# Multiplier for how much sediment a droplet can carry
	set_input(3, "Carry Capacity", ConceptGraphDataType.SCALAR, 
		{"min": 0, "allow_lesser": false})
		
	set_input(4, "Evaporation", ConceptGraphDataType.SCALAR)
	
	# At zero, water will instantly change direction to flow downhill. At 1, water will never change direction
	set_input(5, "Inertia", ConceptGraphDataType.SCALAR, 
		{"min": 0, "max": 1, "value": 0.05, "allow_higher": false, "allow_lesser": false})
		
	set_input(6, "Radius", ConceptGraphDataType.SCALAR)
	set_input(7, "Gravity", ConceptGraphDataType.SCALAR)
	set_input(8, "Erosion Speed", ConceptGraphDataType.SCALAR)
	set_input(9, "Deposit Speed", ConceptGraphDataType.SCALAR)
