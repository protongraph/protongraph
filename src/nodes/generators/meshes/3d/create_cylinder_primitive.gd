...
func _init() -> void:
	...
	display_name = "Create Cylinder Primitive"
	category = "Generators/Meshes/3D"
	description = "Creates a cylinder mesh"

	set_input(0, "Top radius", ConceptGraphDataType.SCALAR, {"value": 1})
	set_input(1, "Bottom radius", ConceptGraphDataType.SCALAR, {"value": 1})
	set_input(2, "Height", ConceptGraphDataType.SCALAR, {"value": 1})
	set_input(3, "Radial segments", ConceptGraphDataType.SCALAR,
		{"value": 64, "step": 1, "min": 1, "allow_lesser": false})

	set_input(4, "Rings", ConceptGraphDataType.SCALAR,
