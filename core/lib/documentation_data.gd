...
class Parameter:
	...
	func set_name(text: String) -> Parameter:
		name = StringUtil.remove_line_breaks(text)
		return self

	func set_description(text: String) -> Parameter:
		description = StringUtil.remove_line_breaks(text)
		return self
	...
	func set_type(val: String) -> Parameter:
		type = StringUtil.remove_line_breaks(val)
		return self

	func add_warning(warning: String, warning_importance := -1) -> Parameter:
		var w = Warning.new()
		w.text = StringUtil.remove_line_breaks(warning)
...
func add_paragraph(text: String) -> void:
	_paragraphs.push_back(StringUtil.remove_line_breaks(text))
...
func add_warning(text: String, importance: int = 0) -> void:
	var w = Warning.new()
	w.text = StringUtil.remove_line_breaks(text)
